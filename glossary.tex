...
{
    È una funzione di attivazione nata per risolvere alcuni problemi delle funzioni di attivazione usate nelle reti MLP, come la sigmoide, in quanto essa ha problemi nella retropropagazione del gradiente. La funzione è definita come
    \[f(x) = \max (0, x)\]
...
\newglossaryentry{pesi}{name={pesi},description={Detti anche sinapsi è il valore di quanto è importante una determinata feature in ingresso al neurone, i pesi vengono modificati da un algoritmo di back propagation come il \gls{sgd}}}